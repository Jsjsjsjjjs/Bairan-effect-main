...
function main() {
  ...
  try {
    ...
    execSync(`${FFMPEG} -threads 1 -loop 1 -i "${OUTPUT_DIR}/last-frame-for-loop.png" -vf "format=yuv420p,fps=60,scale=1080:1920" -c:v libx264 -preset ultrafast -pix_fmt yuv420p -r 60 -t ${freezeDuration} "${OUTPUT_DIR}/freeze-extension.mp4" -y`, { stdio: 'inherit' });
    // Concatenate original + freeze via filter_complex (works on all platforms)
    execSync(`${FFMPEG} -threads 1 -i "${MAIN}" -i "${OUTPUT_DIR}/freeze-extension.mp4" -filter_complex "[0:v]format=yuv420p,fps=60,scale=1080:1920[v0];[1:v]format=yuv420p,fps=60,scale=1080:1920[v1];[v0][v1]concat=n=2:v=1:a=0[out]" -map [out] -c:v libx264 -preset ultrafast -pix_fmt yuv420p -r 60 "${OUTPUT_DIR}/extended-main.mp4" -y`, { stdio: 'inherit' });
    console.log('✓ Done\n');
  } catch (err) {
  ...
  try {
    ...
    execSync(`${FFMPEG} -threads 1 -i "${MIDDLE}" -vf "format=yuv420p,fps=60,scale=1080:1920" -c:v libx264 -preset ultrafast -pix_fmt yuv420p -r 60 -t ${midDur} "${OUTPUT_DIR}/middle-scaled.mp4" -y`, { stdio: 'inherit' });
    ...
    execSync(
      `${FFMPEG} -threads 1 -i "${OUTPUT_DIR}/frozen-bg.mp4" -i "${OUTPUT_DIR}/middle-scaled.mp4"` +
  ...
  try {
    ...
    execSync(
      `${FFMPEG} -threads 1 -i "${OUTPUT_DIR}/extended-main.mp4" -i "${OUTPUT_DIR}/middle-curtain.mp4" -i "${STICKER}" ${audioInput}` +
      ` -filter_complex "` +
      ...
      ` -map [out_v] ${audioMap} -c:v libx264 -preset ultrafast -pix_fmt yuv420p -r 60 -t ${total} "${OUTPUT}" -y`,
      { stdio: 'inherit' }
    );
